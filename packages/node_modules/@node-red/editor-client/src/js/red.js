        ...
        $.ajax({
            ...
            success: function(nodes) {
                if (nodes) {
                    ...
                    var filteredFlows = JSON.parse(JSON.stringify(nodes.flows));
                    var search = window.location.search;
                    if (search) {
                        var searchArrs = search.slice(1).split('=');
                        // 如果有指定对应id，则过滤
                        if (searchArrs[0] = 'activeId') {
                            var activeIds = searchArrs[1].split(';');
                            filteredFlows = filteredFlows.filter(function(node) { return activeIds.indexOf(node.id) > -1 || activeIds.indexOf(node.z) > -1 || node.z === '' } )
                        }
                    }
                    RED.nodes.version(nodes.rev);
                    loader.reportProgress(RED._("event.importFlows"),90 )
                    RED.nodes.import(filteredFlows);
